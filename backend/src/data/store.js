...
async function upsertUser(uid, userPatch) {
  ...
  const existing = (await getUser(uid)) || {
    ...
    displayName: "",
    createdAt: nowIso(),
    ballimal: { health: 70, mood: "content", coins: 0, hunger: 70, happiness: 70, cleanliness: 70 },
    preferences: { species: "cat", colorTheme: "sand", weeklyBudget: 0 },
    unlocks: { species: ["cat"], colorThemes: ["sand"] },
    rewards: { lastWeeklyClaimWeek: "" },
  };
...
async function getExpense(uid, expenseId) {
  if (firebaseReady) {
    const snap = await firestore.collection("users").doc(uid).collection("expenses").doc(expenseId).get();
    if (!snap.exists) return null;
    const data = snap.data();
    if (data.uid !== uid && data.ownerUid !== uid) return null;
    return { id: expenseId, ...data };
  }
  const e = memory.expenses.get(expenseId);
  return e && e.uid === uid ? e : null;
}

async function deleteExpense(uid, expenseId) {
  const existing = await getExpense(uid, expenseId);
  if (!existing) return null;

  if (firebaseReady) {
    await firestore.collection("users").doc(uid).collection("expenses").doc(expenseId).delete();
  } else {
...
async function listExpensesByDateRange(uid, startIsoDate, endIsoDate) {
  if (firebaseReady) {
    const snap = await firestore
      .collection("users")
      .doc(uid)
      .collection("expenses")
      .where("date", ">=", startIsoDate)
      .where("date", "<=", endIsoDate)
      .orderBy("date", "desc")
      .get();
    return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
  }
  return Array.from(memory.expenses.values())
    .filter((e) => e.uid === uid && e.date >= startIsoDate && e.date <= endIsoDate)
    .sort((a, b) => (a.date < b.date ? 1 : -1));
}
...
async function buildMonthlySummary(uid, monthKey) {
  ...
  // Health drops by HEALTH_LOSS_PER_EUR_OVER_BUDGET for each € over the monthly cap (only if budgets exist).
  const budgetOverageEur =
    totalBudget > 0 ? Math.max(0, spentThisMonth - totalBudget) : 0;
  const currentHealth = user?.ballimal?.health ?? 70;
  const nextState = calculateBallimalState({ currentHealth, goalCompletionRate, budgetOverageEur });
